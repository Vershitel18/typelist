...
3. `index_of_unique<Type, List>` &mdash; возвращает индекс единственного вхождения `Type` в `List` (если вхождение не единственное, вызов должен приводить к ошибке компиляции);
4. `flatten<List>` &mdash; разворачивает все рекурсивно вложенные тайплисты в один одномерный;
5. `merge_sort<Compare, List>` &mdash; возвращает копию `List`, но с элементами, отсортированными компаратором `Compare` (он принимает два типа, и возвращает `true`, если первый должен идти до второго).
...
Не добавляйте в решение неиспользуемый код.
